...
rdatasSVCB :: MonadParser Token s m => [(TYPE, m RData)]
rdatasSVCB =
    [ (HTTPS, rdataSVCB)
    , (SVCB, rdataSVCB)
    ]
...
-- >>> parseSVCP s = runParser' (svcParam $ esstr s) mempty >>= \((k, v), _) -> maybe (Left "no-value") (Right . (,) k) $ fromSvcParamValue v
--
...
svcParam :: MonadParser t s m => EString -> m (SvcParamKey, SvcParamValue)
svcParam es = splitKeyStr es invalidK $ \sk sv -> do
