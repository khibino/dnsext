...
pprWorkerStats _pn ops = do
    ...
    let isBkStat p (_n, _ws, (bks, _bcx, _diff)) = p bks
        ...
        pprEnq  p (wn, _ws, wbs@(_, BkContext (BcEnqueue _q dox) _, _))
            | p dox  = ((show wn ++ ":" ++ pprBlockingStat wbs) :)
        pprEnq _p  _  = id
        pprEnqs
            | null pp    = "no workers"
            | otherwise  = pp
          where h2  = foldr (pprEnq (== H2))  [] blockings
                dot = foldr (pprEnq (== DoT)) [] blockings
                xs  = foldr (pprEnq (\x -> x /= H2 && x /= DoT)) [] blockings
...
instance Show BkCause where
    ...
    show (BcEnqueue q dox)  = "enqueue: " ++ show q ++ ": " ++ show dox
    show  BcLog             = "logging"
    show  BcIO              = "I/O"
...
noopWorkerStat =
    ...
    , setUnblocked     = return ()
    , getBlockingStat  = return (BsBlocking, BkContext BcDequeueReq "<noop>", DiffT (-1))
    }
{- FOURMOLU_ENABLE -}
...
getWorkerStatOP = do
  ...
  where
    ...
    mkBkStore = do
        ...
        return WBStore{wbkStatRef = ref, wbkContext = BkContext BcDequeueReq "<init>"}
...
bracketEnqueue wstat q dox note = bracketBlocking wstat (BkContext (BcEnqueue q dox) note)
