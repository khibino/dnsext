...
delegationFallbacks_ eh fh qparallel disableV6NS dc dnssecOK ah d0@Delegation{..} name typ = do
  ...
  where
    ...
    stepNS (ns, tyAx) fbs d aa = do
        res  <- resolveNS' ns tyAx
        d1   <- fillCachedDelegation d
        let fallbacks' as = fbs d1 $ aa . ((show ns, as) :)
            left  e    = eh' (unwords [e, "for resolving", show ns, show tyAx]) >> fallbacks' []
            right axs  = randomizedPermN [(ip, 53) | (ip, _) <- axs] <&> NE.toList >>= \ps -> fallbacksAx d1 ps $ fallbacks' ps
