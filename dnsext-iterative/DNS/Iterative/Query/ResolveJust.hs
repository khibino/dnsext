...
resolveExactDC dc n typ
  ...
  where
    ...
    request nss@Delegation{..} = do
        checkEnabled <- lift getCheckEnabled
...
fillsDNSSEC nss d = do
    ...
    when (checkEnabled && chainedStateDS filled && null delegationDNSKEY) $ do
...
getCheckEnabled :: Monad m => ContextT m Bool
getCheckEnabled = notCD <$> lift (asks requestCD_)
  where
    notCD NoCheckDisabled = True
    notCD CheckDisabled   = False
