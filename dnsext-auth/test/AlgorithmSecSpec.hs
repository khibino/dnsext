...
spec = describe "authoritative algorithm" $ do
    ...
    it "can answer an non-existing domain" $ do
        ...
        authority ans `shouldSatisfy` include "example.jp." RRSIG
        authority ans `shouldSatisfy` include "ns.in.example.jp." NSEC
        authority ans `shouldSatisfy` include "ns.in.example.jp." RRSIG
        length (additional ans) `shouldBe` 0
        flags ans `shouldSatisfy` authAnswer
