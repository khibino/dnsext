...
module Iterative (iterativeQuery, getRemoteV6) where
...
getRemoteV6 = do
  ...
  where
    ...
    s6 na6      xs = [(IPv6 a,  p) | (a, p) <- nlist na6] ++ xs
    takeV6 = delegationEntry ax (\_ _ xs -> xs) a6 (\_ xs -> xs) (\_ xs -> xs) s6
{- FOURMOLU_ENABLE -}
